...
function moneyMil(n: number) {
  ...
  return new Intl.NumberFormat("vi-VN", { maximumFractionDigits: 1 }).format(val) + " Tr";
}
...
export default function AdminDashboardPage() {
  ...
  const kpis = p1Data?.kpis || { 
    totalCases: 0, paidCases: 0, totalRevenue: 0, 
    totalCost: 0, totalNetRevenue: 0, paidRate: 0, actualNetRevenue: 0 
  };
        ...
        <div className="border-b border-black/5 bg-white px-4 py-6 shadow-sm dark:border-white/10 dark:bg-neutral-900">
          <div className="mx-auto max-w-7xl">
            <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-neutral-500">
                  Thống kê toàn bộ dòng tiền, bảng xếp hạng các nguồn gửi mẫu và biểu đồ doanh thu.
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-3">
                <SelectPill value={p1Service} onChange={setP1Service} options={SERVICE_OPTIONS} />
                <SelectPill value={p1Month} onChange={setP1Month} options={MONTH_OPTIONS} />
              </div>
            </div>
            ...
            <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
              ...
              <KpiCard title="Lợi nhuận dự kiến (Net)" value={money(kpis.totalNetRevenue)} sub="Toàn bộ ca" icon="⚖️" theme="amber" />
              
              {/* ✅ THẺ LỢI NHUẬN THỰC TẾ CẬP NHẬT THEO LOGIC MỚI */}
              <KpiCard title="Kế toán đã thu về" value={money(kpis.actualNetRevenue)} sub="Chỉ tính các ca đã TT" icon="💎" highlight />
            ...
            <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
              ...
              <Card title={`Bảng xếp hạng Nguồn ${p1Month !== "ALL" ? `(Tháng ${p1Month})` : "(Toàn thời gian)"}`}>
                <div className="max-h-[350px] overflow-auto rounded-xl border border-black/5 dark:border-white/10">
                  <table className="w-full border-collapse text-sm">
                    ...
                    <tbody>
                      {bySource.length === 0 ? (
                        <tr><td colSpan={4} className="py-8 text-center text-neutral-500">Không có dữ liệu</td></tr>
                      ) : (
              ...
              <Card title={`Biểu đồ Dòng tiền các tháng ${p1Service ? `(${p1Service})` : ""}`}>
                <div className="h-[350px] w-full pt-4">
                  <ResponsiveContainer>
                    <BarChart data={monthlyTrend} margin={{ top: 20, right: 0, left: 0, bottom: 0 }}>
                      ...
                      <Tooltip formatter={(v: any) => moneyMil(Number(v || 0))} cursor={{ fill: 'transparent' }} />
                      <Legend />
                      <Bar dataKey="netRevenue" name="LN Dự kiến (Net)" fill={COLOR_NET} radius={[4, 4, 0, 0]}>
                        <LabelList dataKey="netRevenue" position="top" formatter={(v: any) => moneyMil(v)} style={{ fontSize: '10px', fill: '#525252', fontWeight: 600 }} />
                      </Bar>
                      <Bar dataKey="cost" name="Chi phí (Cost)" fill={COLOR_COST} radius={[4, 4, 0, 0]}>
                        <LabelList dataKey="cost" position="top" formatter={(v: any) => moneyMil(v)} style={{ fontSize: '10px', fill: '#525252', fontWeight: 600 }} />
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
        ...
        <div className="mx-auto mt-8 max-w-7xl px-4">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-neutral-500">
                So sánh doanh thu và xếp hạng giữa các gói xét nghiệm trong cùng một loại dịch vụ.
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-3">
              <SelectPill value={p2Service} onChange={setP2Service} options={SERVICE_OPTIONS.filter(o => o.value !== "")} />
              <SelectPill value={p2Month} onChange={setP2Month} options={MONTH_OPTIONS} />
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <Card title={`So sánh LN dự kiến các gói ${p2Service} ${p2Month !== "ALL" ? `(${p2Month})` : ""}`}>
              <div className="h-[400px] w-full pt-4">
                <ResponsiveContainer>
                  <BarChart data={byService} layout="vertical" margin={{ top: 0, right: 50, left: 40, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" horizontal={false} />
                    <XAxis type="number" tickFormatter={(v) => moneyMil(v)} tick={{ fontSize: 11 }} />
                    <YAxis type="category" dataKey="serviceCode" width={80} tick={{ fontSize: 11, fontWeight: 'bold' }} />
                    <Tooltip formatter={(v: any) => moneyMil(Number(v || 0))} cursor={{ fill: 'transparent' }} />
                    <Legend />
                    <Bar dataKey="netRevenue" name="LN Dự kiến (Net)" fill={COLOR_REV} radius={[0, 4, 4, 0]} barSize={20}>
                      <LabelList dataKey="netRevenue" position="right" formatter={(v: any) => moneyMil(v)} style={{ fontSize: '10px', fill: '#525252', fontWeight: 600 }} />
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ...
            <Card title={`Bảng xếp hạng gói ${p2Service}`}>
              <div className="max-h-[400px] overflow-auto rounded-xl border border-black/5 dark:border-white/10">
                <table className="w-full border-collapse text-sm">
                  <thead className="sticky top-0 z-10 bg-white/95 backdrop-blur dark:bg-neutral-900/90">
                    <tr className="text-xs text-neutral-500">
                      <Th>Mã Gói</Th>
                    ...
                    {byService.length === 0 ? (
                      ...
                      byService.map((x: any) => (
                        <tr key={x.serviceCode} className="border-t border-black/5 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/5">
                          <Td className="font-bold whitespace-nowrap">{x.serviceCode || "—"}</Td>
...
function Card({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col rounded-3xl border border-black/5 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-neutral-900">
      <div className="mb-4 text-sm font-bold text-neutral-900 dark:text-white">{title}</div>
    ...
    return (
      <div className="flex flex-col justify-between rounded-3xl bg-gradient-to-br from-blue-500 to-blue-600 p-4 text-white shadow-md transition-transform hover:scale-[1.02]">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="grid h-10 w-10 place-items-center rounded-2xl bg-white/20 ring-1 ring-white/20"><span className="text-lg">{icon}</span></div>
        </div>
      </div>
    );
  }
  return (
    <div className="flex flex-col justify-between rounded-3xl border border-black/5 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className={cn("grid h-10 w-10 place-items-center rounded-2xl bg-gradient-to-br ring-1 ring-black/5", t.iconBg)}><span className="text-lg">{icon}</span></div>
      </div>
    </div>
  );
}
...
function Th({ children, right }: any) {
  return <th className={cn("px-4 py-3 text-left font-semibold border-b border-black/5", right && "text-right")}>{children}</th>;
}
