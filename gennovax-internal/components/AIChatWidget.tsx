...
export default function AIChatWidget() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      role: "ai",
      content: "Chào quản lý! Tôi là trợ lý phân tích dữ liệu. Bạn muốn thống kê doanh thu hay số liệu nào hôm nay?",
    },
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (res.success) {
        setMessages((prev) => [...prev, { id: Date.now().toString(), role: "ai", content: res.answer }]);
      } else {
      ...
      setMessages((prev) => [
        ...prev,
        { id: Date.now().toString(), role: "ai", content: "❌ Xin lỗi, hệ thống AI đang bận hoặc lỗi: " + error.message },
      ]);
    } finally {
      setLoading(false);
  ...
  const handleClear = () => {
    setMessages([{ id: "welcome", role: "ai", content: "Đã xóa cuộc hội thoại. Bạn cần hỗ trợ gì thêm?" }]);
  };
        ...
        <div className="mb-4 w-xl h-[80vh]  bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-neutral-200 transition-all duration-300">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-4 text-white flex justify-between items-center shadow-md">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-lg leading-tight">AI Analytics</h3>
                <p className="text-xs text-blue-100">Trợ lý trực tuyến</p>
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={handleClear} className="text-white/80 hover:text-white text-sm" title="Xóa chat">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
              </button>
              <button onClick={() => setIsOpen(false)} className="text-white hover:text-neutral-200 text-2xl leading-none">&times;</button>
            </div>
          </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto bg-[#F0F2F5] flex flex-col gap-5">
            {messages.map((msg) => {
              ...
              return (
                <div key={msg.id} className={`flex gap-2 items-end ${isUser ? "flex-row-reverse" : "flex-row"}`}>
                  
                  {/* Avatar */}
                  <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center shadow-sm ${isUser ? "bg-indigo-100" : "bg-white"}`}>
                    {isUser ? (
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-indigo-600"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" /></svg>
                    ) : (
