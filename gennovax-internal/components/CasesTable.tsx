...
export default function CasesTable({
  ...
  const handleDelete = async (
    ...
    if (
      !window.confirm(
        `Bạn có chắc chắn muốn xóa ca của bệnh nhân "${patientName || "Không tên"}" không? Hành động này không thể hoàn tác.`,
      )
    )
      return;
    ...
    <>
      <div className="rounded-3xl bg-white shadow-sm ring-1 ring-black/5">
          ...
          >
            ...
            <thead className="sticky top-0 z-10">
              <tr className="border-b bg-white text-neutral-600">
                ...
                {isAccountingAdmin && (
                  <th className={`${thBase} bg-neutral-50 `}>
                  Kiểu TT
                </th>
                )}
                {isAccountingAdmin && (
                ...
                rows.map((r, idx) => (
                  ...
                  >
                    ...
                    {isAccountingAdmin && (
                      ...
                      {( (r.collectedAmount || 0) - (r.costPrice || 0) ).toLocaleString()}
                    </td>
                    )}
                    {/* Cột Hành động */}
        ...
        <div className="flex items-center justify-between px-4 py-3 text-[11px] text-neutral-500 border-t bg-neutral-50">
          <div>
            Tip: Cuộn ngang chỉ ở bảng. Bấm “Xem” để xem lịch sử chỉnh sửa. Bấm
            vào hàng để mở chi tiết ca.
          </div>
          <div>
