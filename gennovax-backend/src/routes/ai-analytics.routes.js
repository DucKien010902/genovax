...
try {
  if (fs.existsSync(knowledgePath)) {
    ...
    console.warn(`⚠️ Cảnh báo: Không tìm thấy file kiến thức tại ${knowledgePath}`);
  }
} catch (err) {
...
router.post("/ask", async (req, res, next) => {
  try {
    ...
    res.status(500).json({ source: "error", answer: "Xin lỗi, hiện tại tôi đang không thể xử lý dữ liệu từ DB." });
  }
});
...
router.post("/ask-info", async (req, res) => {
  try {
    ...
    res.status(500).json({ source: "error", answer: "Hệ thống tư vấn đang bận, vui lòng thử lại sau" });
  }
});
