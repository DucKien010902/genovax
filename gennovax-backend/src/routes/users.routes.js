...
router.post("/", requireRole("admin"), async (req, res, next) => {
  try {
    ...
    const newUser = await User.create({ name, email, passwordHash, role: role || "staff" });
...
router.patch("/:id", requireRole("admin"), async (req, res, next) => {
  try {
    ...
    const updated = await User.findByIdAndUpdate(req.params.id, updateData, { new: true }).select("-passwordHash").lean();
    res.json(updated);
  } catch (e) {
    next(e);
  }
