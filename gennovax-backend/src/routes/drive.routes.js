...
router.get("/list", async (req, res) => {
  try {
    ...
    stream.on("data", function (obj) {
      ...
      // Nếu là File
      else if (obj.name !== prefix) { // Bỏ qua file rỗng 0 byte dùng để tạo thư mục
...
router.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    const safeFileName = Date.now() + "_" + originalname.replace(/[^a-zA-Z0-9.\-_]/g, '');
    const objectName = `${currentPath}${safeFileName}`;

    await minioClient.fPutObject(BUCKET_NAME, objectName, tempPath, { "Content-Type": mimetype });
